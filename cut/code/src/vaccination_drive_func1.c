...
int beneficiary_file_to_list()
{
	FILE *pc;
	bt c1;
	if((pc=fopen("../data/beneficiary.data","rb"))==NULL)
	{
		printf("\n File is not found\n");
		return EXIT_FAILURE;
	}
	...
	while(!feof(pc))
	{
		...
		new->beneficiary_age=c1.beneficiary_age;
		new->no_of_doses=c1.no_of_doses;
...
int beneficiary_list_to_file()
{
	...
	FILE *pc1;
	if((pc1=fopen("../data/beneficiary.data","wb"))==NULL)
	{
		printf("\n File is not found\n");
		return EXIT_FAILURE;
	}
	for(ptr=start;(ptr);ptr=ptr->next)
	{
		...
		c2.beneficiary_age=ptr->beneficiary_age;
		c2.no_of_doses=ptr->no_of_doses;
		//strcpy(c2.select_centre,ptr->select_centre);
		strcpy(c2.password,ptr->password);
		fwrite(&c2,sizeof(bt),1,pc1);
